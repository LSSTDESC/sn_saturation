...
\bibliographystyle{desc-rtex/bst/apj}
...
The command \code{make main} compiles \code{main.tex} using the  \code{lsstdescnote.cls} style.
...
There are a number of useful \LaTeX\xspace commands provided by \code{desc-tex}, which should be automatically downloaded when \code{make} is invoked.
...
The \code{desc-tex} macros also contain some conventional scientific units like \angstrom, \GeV, \Msun, etc. and some editorial tools for highlighting \FIXME{issues}, \CHECK{text to be checked}, \COMMENT{comments}, and \NEW{new additions}.
...
\caption{An example figure: the LSST DESC logo, copied from \code{desc-tex/logos/desc-logo.png} into \code{figures/example.png}. \label{fig:example}}
\end{figure}
